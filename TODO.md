...
- [x] **운영 Webhook 이벤트 및 Hermes Generic HMAC V2 지원** — `issue.status_changed`, `task.failed`, `runtime.offline`, `runtime.recovered`, `server.ready` vocabulary와 공통 envelope 검증을 추가. 지원 이벤트 전체 fan-out, 이벤트별 strict filter matching, Hermes timestamp 기반 V2 HMAC headers, create/update 이벤트 검증과 회귀 테스트를 구현.

- [x] **Webhook 기반 장애 복구 및 시작 시 상태 화해** — 모든 fresh task 실패 경로를 retry/fallback/final disposition 파이프라인으로 통합하고 redacted `task.failed` payload, runtime offline/recovered 및 server ready lifecycle event를 구현. daemon/server boot UUID와 DB 기반 중복 방지, 원자적 `task_remediation` claim·자동 복구 budget·marker comment, `multica issue remediate`, Hermes final-failure filter·startup reconciliation hook·운영 runbook을 추가하고 동시 호출·active sibling·fallback·재할당·재시작 회귀 테스트를 보강.
...
- [x] **Antigravity Planning Agent task-scoped auth token 정식 수정** — Migration 073으로 `task_token` 테이블과 `mat_` token lifecycle을 추가하고 daemon task claim 응답에서 mint/persist/반환. daemon은 owner PAT fallback 없이 task token을 fail-closed로 agent에 주입하며, agent-context CLI도 saved PAT로 fallback하지 않음. Codex shell에는 daemon-managed allow-list policy로 현재 task의 승인된 `MULTICA_*`/custom env만 전달하고 user config override를 차단. task 완료/실패/취소 시 revoke 및 claim/auth/daemon/CLI/Codex shell 회귀 테스트를 포함.
...
- [x] **Webhook Test Delivery payload 보정** — Settings Webhooks의 Test Delivery가 구독 목록의 첫 이벤트를 선택하고 `issue.status_changed`, `task.failed`, `runtime.offline`, `runtime.recovered`, `server.ready` 각각에 유효한 공통 envelope와 event별 sample 필드를 생성하도록 수정하고 회귀 테스트를 추가.
