...
<Callout type="info">
Daemon-managed agents do not receive this owner PAT. Each claimed task receives
a short-lived, task-scoped `mat_` token through `MULTICA_TOKEN`; commands run in
an agent task fail closed if that token is missing or has another token type.
The CLI never falls back to the saved profile PAT from an agent execution
context.
</Callout>
...
| `multica issue remediate <id> ...` | Atomically rerun, reassign-and-rerun, or block after a final task failure |
...
The daemon keeps the selectable runtime model catalog in an editable JSON text file. It creates `~/.multica/runtime-models.json` on the first model-list request; named CLI profiles use `~/.multica/profiles/<profile>/runtime-models.json`. Set `MULTICA_RUNTIME_MODELS_CONFIG` to an absolute path to use a different file.

The file is read and validated for every model-list request, so edits appear in the agent settings UI without rebuilding Multica or restarting the daemon. Each provider has a `models` array whose entries require `id` and `label` and may include `provider`, `default`, and `thinking` metadata. Invalid JSON, duplicate IDs, multiple defaults, or an unsupported default thinking level are reported as a model-list error instead of silently using stale values.
