...
|---|---|---|
| **Daemon** | `~/.multica/daemon.log` (rotating structured log), `~/.multica/daemon.err.log` (crash output), or foreground stdout | `multica daemon logs -f --lines 100` |
...
Background `daemon.log` rotates at 20 MB by default, retaining five gzip-compressed backups for up to 30 days. Tune this with `MULTICA_DAEMON_LOG_MAX_SIZE_MB`, `MULTICA_DAEMON_LOG_MAX_BACKUPS`, and `MULTICA_DAEMON_LOG_MAX_AGE_DAYS` (positive integers only).
