...
| **Antigravity** | Google | ⚠️ 接受 resume flag，但不返回 session id | ❌ | `.agent_context/skills/` （fallback）| 动态发现 |
...
Google 出品。通过 `agy --print` 非交互模式运行。模型列表从 `agy models` 动态发现。CLI 接受 conversation resume flag，但 print mode 不返回 session id，所以在它暴露结构化 session/streaming 协议前，应按一次性执行工具看待。
...
| ❌ 无 | Gemini、Antigravity | Gemini CLI 无 resume 机制；Antigravity print mode 不返回可复用 session id |
...
| Antigravity | `.agent_context/skills/` | ⚠️ 通用 fallback |

fallback 路径对应的工具是否真的读取这个目录，取决于工具本身的文档——没保证。如果你的 skill 对 Gemini / Hermes / OpenClaw / Antigravity 没起效，先查这个问题。
