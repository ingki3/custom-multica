...
| **Antigravity** | Google | ⚠️ Resume flag accepted, session id not returned | ❌ | `.agent_context/skills/` (fallback) | Dynamic discovery |
...
From Anthropic. **First choice for new users** — the most complete feature set: session resumption actually works, it's the **only one of the 12 that truly reads MCP configuration**, and it supports fine-tuning flags like `--max-turns` and `--append-system-prompt`. Requires an Anthropic API key.
...
From Google. Uses the `agy` CLI in non-interactive print mode (`agy --print`). Model selection is discovered dynamically from `agy models`. The CLI accepts a conversation resume flag, but print mode does not return a session id, so treat it as effectively one-shot until the CLI exposes a structured session/streaming protocol.
...
| ❌ None | Gemini, Antigravity | Gemini has no resume mechanism; Antigravity print mode does not return a reusable session id |
...
| Antigravity | `.agent_context/skills/` | ⚠️ Generic fallback |

Whether a fallback-path tool actually reads this directory depends on the tool's own documentation — no guarantees. If your skills aren't taking effect for Gemini / Hermes / OpenClaw / Antigravity, check this first.
