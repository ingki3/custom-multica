...
| `MULTICA_RUNTIME_MODELS_CONFIG` | profile-local `runtime-models.json` | Absolute path overriding the editable runtime model catalog; the file is re-read on every model-list request |
