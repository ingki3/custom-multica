...
// supplied workspace/task IDs. It is side-effect free and is used by daemon GC
// guards before Prepare has had a chance to mkdir the directory.
...
// ReuseCustomFolder wraps a user-specified project folder into an Environment.
// It validates the folder exists and is writable, writes context files
// (.agent_context/), but does NOT create directory structure or overwrite
// existing runtime config files (CLAUDE.md, AGENTS.md, GEMINI.md).
// Returns nil if the folder doesn't exist or isn't writable.
func ReuseCustomFolder(folderPath, provider, codexVersion string, task TaskContextForEnv, logger *slog.Logger) *Environment {
	info, err := os.Stat(folderPath)
	if err != nil || !info.IsDir() {
		logger.Warn("execenv: custom working folder not found", "path", folderPath, "error", err)
		return nil
	}

	// Validate writability.
	testFile := filepath.Join(folderPath, ".multica_write_test")
	if err := os.WriteFile(testFile, []byte(""), 0o644); err != nil {
		logger.Warn("execenv: custom working folder not writable", "path", folderPath, "error", err)
		return nil
	}
	os.Remove(testFile)

	env := &Environment{
		RootDir: "", // empty = signals "don't GC this directory"
		WorkDir: folderPath,
		logger:  logger,
	}

	// Write context files (.agent_context/issue_context.md + skills).
	if err := writeContextFiles(folderPath, provider, task); err != nil {
		logger.Warn("execenv: write context files to custom folder failed", "error", err)
	}

	// Handle Codex provider.
	if provider == "codex" {
		codexHome := filepath.Join(folderPath, ".agent_context", "codex-home")
		if err := prepareCodexHomeWithOpts(codexHome, CodexHomeOptions{CodexVersion: codexVersion}, logger); err != nil {
			logger.Warn("execenv: prepare codex-home in custom folder failed", "error", err)
		} else {
			env.CodexHome = codexHome
			if err := writeCodexWorkspaceSkills(codexHome, task.AgentSkills); err != nil {
				logger.Warn("execenv: write codex skills in custom folder failed", "error", err)
			}
		}
	}
