...
func (d *Daemon) handleModelList(ctx context.Context, rt Runtime, requestID string) {
	...
	type modelWire struct {
		...
		Thinking *agent.ModelThinking `json:"thinking,omitempty"`
	}
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, taskLog *slog.Logger) (TaskResult, error) {
	...
	// Determine working directory with 3-tier priority:
	// 1. Project-level working_folder (user's local project directory)
	// 2. PriorWorkDir from a previous task on the same (agent, issue) pair
	// 3. New isolated environment under ~/multica_workspaces/
	...
	if task.WorkingFolder != "" {
		configFile := execenv.RuntimeConfigFilename(provider)
		if configFile != "" {
			configPath := filepath.Join(env.WorkDir, configFile)
			if _, err := os.Stat(configPath); err != nil {
				...
				d.logger.Info("execenv: preserving existing runtime config", "file", configFile)
			}
		}
	} else {
		if err := execenv.InjectRuntimeConfig(env.WorkDir, provider, taskCtx); err != nil {
