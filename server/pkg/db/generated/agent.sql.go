...
const claimTaskFailureProcessing = `-- name: ClaimTaskFailureProcessing :one
UPDATE agent_task_queue
...
func (q *Queries) ClaimTaskFailureProcessing(ctx context.Context, id pgtype.UUID) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, claimTaskFailureProcessing, id)
	var i AgentTaskQueue
	err := row.Scan(
		&i.ID,
		&i.AgentID,
...
const getTaskChildByParent = `-- name: GetTaskChildByParent :one
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, last_heartbeat_at, trigger_summary, failure_processing_started_at, failure_handled_at FROM agent_task_queue
WHERE parent_task_id = $1
LIMIT 1
`

func (q *Queries) GetTaskChildByParent(ctx context.Context, parentTaskID pgtype.UUID) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, getTaskChildByParent, parentTaskID)
	var i AgentTaskQueue
	err := row.Scan(
		&i.ID,
		&i.AgentID,
...
const markTaskFailureHandled = `-- name: MarkTaskFailureHandled :exec
UPDATE agent_task_queue
SET failure_handled_at = now(), failure_processing_started_at = NULL
WHERE id = $1 AND failure_handled_at IS NULL
`

func (q *Queries) MarkTaskFailureHandled(ctx context.Context, id pgtype.UUID) error {
	_, err := q.db.Exec(ctx, markTaskFailureHandled, id)
	return err
}
...
const releaseTaskFailureProcessing = `-- name: ReleaseTaskFailureProcessing :exec
UPDATE agent_task_queue
SET failure_processing_started_at = NULL
WHERE id = $1 AND failure_handled_at IS NULL
`

func (q *Queries) ReleaseTaskFailureProcessing(ctx context.Context, id pgtype.UUID) error {
	_, err := q.db.Exec(ctx, releaseTaskFailureProcessing, id)
	return err
}
...
func (q *Queries) RequeueTask(ctx context.Context, id pgtype.UUID) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, requeueTask, id)
	var i AgentTaskQueue
	err := row.Scan(
		&i.ID,
		&i.AgentID,
