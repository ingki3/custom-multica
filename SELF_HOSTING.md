...
## Option A — Native Setup (No Docker)

Run Multica directly on your machine without Docker. Requires PostgreSQL 17 with pgvector installed natively.
...
**Create the database user** (if your system doesn't have a `multica` role):

```bash
createuser -s multica
# Or with a password:
...
`make selfhost-native` automatically:
1. Creates `.env` from the example with a random `JWT_SECRET`
2. Verifies native PostgreSQL is running
3. Installs frontend dependencies
4. Builds the Go backend binary
...
Use Docker Compose to run the full stack in containers.
...
**Native setup:** Press `Ctrl+C` in the terminal where `make selfhost-native` is running. Backend and frontend stop immediately. PostgreSQL continues running via your OS service manager.
