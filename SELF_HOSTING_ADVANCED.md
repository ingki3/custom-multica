...
### Native PostgreSQL (Recommended for No-Docker Setup)
...
```bash
# Create a superuser role (adjust as needed)
psql -U postgres -c "CREATE ROLE multica WITH LOGIN PASSWORD 'multica' SUPERUSER;"

# Create the database
createdb -U multica multica

# Verify pgvector is available
psql -U multica -d multica -c "CREATE EXTENSION IF NOT EXISTS vector;"
```
...
If you prefer to use an existing PostgreSQL instance (e.g., RDS, Cloud SQL), ensure the pgvector extension is available:

```sql
CREATE EXTENSION IF NOT EXISTS vector;
```

Set `DATABASE_URL` in your `.env` to point to your instance.

### Running Migrations Manually

Migrations run automatically on `make selfhost-native` and Docker Compose startup. To run them manually:
...
The recommended way to self-host without Docker:

```bash
make selfhost-native       # Production mode (builds binaries)
make selfhost-native-dev   # Dev mode (hot-reload)
```

If you prefer manual steps:

**Prerequisites:** Go 1.26+, Node.js 20+, pnpm 10.28+, PostgreSQL 17 with pgvector (running natively).
