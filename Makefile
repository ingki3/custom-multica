.PHONY: help makehelp dev server daemon cli multica build test migrate-up migrate-down sqlc seed clean setup start stop check worktree-env setup-main start-main stop-main check-main setup-worktree start-worktree stop-worktree check-worktree db-up db-down db-reset selfhost selfhost-build selfhost-stop selfhost-native selfhost-native-dev
...
selfhost-native: ## Self-host without Docker: build and run backend + frontend natively
	@if [ ! -f .env ]; then \
		echo "==> Creating .env from .env.example..."; \
		cp .env.example .env; \
		JWT=$$(openssl rand -hex 32); \
	...
	@echo ""
	@echo "==> Starting Multica natively..."
	@echo "  Backend:  http://localhost:$${PORT:-8080}"
	@echo "  Frontend: http://localhost:$${FRONTEND_PORT:-3000}"
	@echo ""
	@trap 'kill 0' EXIT; \
		(cd server && ./bin/server) & \
		(cd apps/web && REMOTE_API_URL=http://localhost:$${PORT:-8080} pnpm start) & \
		wait

selfhost-native-dev: ## Self-host without Docker in dev mode (hot-reload)
	@if [ ! -f .env ]; then \
		echo "==> Creating .env from .env.example..."; \
		cp .env.example .env; \
		JWT=$$(openssl rand -hex 32); \
	...
	@echo ""
	@echo "==> Starting Multica in dev mode (no Docker)..."
	@echo "  Backend:  http://localhost:$${PORT:-8080}"
	@echo "  Frontend: http://localhost:$${FRONTEND_PORT:-3000}"
	@echo ""
	@trap 'kill 0' EXIT; \
...
db-up: ## Start PostgreSQL (Docker container or verify native is running)
	@if pg_isready -h localhost -p $(POSTGRES_PORT) -U $(POSTGRES_USER) > /dev/null 2>&1; then \
		echo "✓ Native PostgreSQL already running on localhost:$(POSTGRES_PORT)"; \
	elif command -v docker > /dev/null 2>&1; then \
		$(COMPOSE) up -d postgres; \
	else \
		echo "✗ No PostgreSQL on localhost:$(POSTGRES_PORT) and Docker is not installed."; \
		echo "  Start PostgreSQL manually or install Docker."; \
		exit 1; \
	fi

db-down: ## Stop PostgreSQL (Docker container only; no-op for native)
	@if command -v docker > /dev/null 2>&1 && docker compose ps --status running 2>/dev/null | grep -q postgres; then \
		$(COMPOSE) down; \
	else \
		echo "No Docker PostgreSQL container running. If using native PostgreSQL, stop it via your OS service manager."; \
	fi
...
db-reset: ## Drop and recreate the current env's database, then re-run all migrations
	...
	@if pg_isready -h localhost -p $(POSTGRES_PORT) -U $(POSTGRES_USER) > /dev/null 2>&1 && ! (command -v docker > /dev/null 2>&1 && docker compose ps --status running 2>/dev/null | grep -q postgres); then \
