...
- **`main`** and **`dev`** branches are protected — no direct push allowed. All changes go through Pull Requests.
- **Feature branches** must be created from `dev` and merged back to `dev` via PR.
- When `dev` is stable and ready for release, create a PR from `dev` → `main`.
- Feature branch naming: `feat/<short-description>`, `fix/<short-description>`, `refactor/<short-description>`.
- Always push to a feature branch, never directly to `dev` or `main`.
