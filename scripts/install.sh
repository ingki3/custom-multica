...
# ---------------------------------------------------------------------------
# Docker check (returns 0 if Docker is available and running, 1 otherwise)
# ---------------------------------------------------------------------------
check_docker() {
...
# ---------------------------------------------------------------------------
# Native prerequisites check (Go, Node, pnpm, PostgreSQL)
# ---------------------------------------------------------------------------
check_native_prereqs() {
  ...
  if [ ${#missing[@]} -gt 0 ]; then
    fail "Missing prerequisites for native setup: ${missing[*]}
Please install: Go 1.26+, Node.js 20+, pnpm 10.28+"
  fi

  if ! command_exists psql; then
    fail "PostgreSQL client (psql) not found. Install PostgreSQL 17 with pgvector.

macOS:   brew install postgresql@17 pgvector
Ubuntu:  sudo apt install postgresql-17 postgresql-17-pgvector"
  fi
...
Start PostgreSQL first:
  ...
  ok "Native prerequisites available (Go, Node.js, pnpm, PostgreSQL)"
}

# ---------------------------------------------------------------------------
# Server setup — shared: clone repo, generate .env
# ---------------------------------------------------------------------------
setup_server_common() {
...
# ---------------------------------------------------------------------------
# Server setup — native mode (no Docker)
# ---------------------------------------------------------------------------
setup_server_native() {
  setup_server_common

  # Ensure PostgreSQL and create database
  ...
  # Write a PID file so selfhost-stop can find them
  local pid_dir="$INSTALL_DIR/.pids"
  mkdir -p "$pid_dir"

  (cd server && ./bin/server) &
  echo $! > "$pid_dir/backend.pid"

  (cd apps/web && REMOTE_API_URL="http://localhost:${port}" pnpm start) &
  ...
  if [ "$ready" = true ]; then
    ok "Multica server is running (native mode)"
  else
    warn "Server is still starting. Check backend logs at server/tmp/ or re-run manually."
    echo ""
  fi
}
...
run_with_server() {
  ...
  local server_mode="native"
  if check_docker; then
    server_mode="docker"
  else
    info "Docker not available. Setting up natively..."
    check_native_prereqs
  fi
...
run_stop() {
  ...
  if [ -d "$INSTALL_DIR" ]; then
    ...
    # Stop native-mode processes via PID files
    local pid_dir="$INSTALL_DIR/.pids"
    if [ -d "$pid_dir" ]; then
    ...
    if [ -f docker-compose.selfhost.yml ] && command_exists docker && docker info >/dev/null 2>&1; then
      if docker compose -f docker-compose.selfhost.yml ps --status running 2>/dev/null | grep -q .; then
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo "                  Uses Docker if available, otherwise builds natively"
        echo "                  (requires Go, Node.js, pnpm, PostgreSQL 17)"
